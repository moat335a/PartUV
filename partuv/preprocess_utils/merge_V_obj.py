...
def weld_vertices(mesh: trimesh.Trimesh, epsilon: float = 1e-5, keep: str = "first") -> trimesh.Trimesh:
    ...
    # Optional: also remove duplicate/degenerate faces just in case (older trimesh lacks remove_duplicate_faces)
    if hasattr(m_out, "remove_duplicate_faces"):
        m_out.remove_duplicate_faces()
    else:
        # Manual dedup by collapsing faces with identical vertex sets
        faces = m_out.faces
        keys = np.sort(faces, axis=1)
        _, keep_idx = np.unique(keys, axis=0, return_index=True)
        if len(keep_idx) != len(faces):
            m_out.update_faces(keep_idx)
            m_out.remove_unreferenced_vertices()
    if hasattr(m_out, "remove_degenerate_faces"):
